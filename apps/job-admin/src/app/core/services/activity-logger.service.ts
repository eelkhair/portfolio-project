 ...
 *
 * For standalone `info / warn / error` calls (auth lifecycle, mode toggle),
 * we probe `trace.getActiveSpan()` — if the call is happening inside an
 * already-active context, the log inherits its trace id automatically.
 *
 * Falls back to `console.*` when Faro hasn't initialized (local dev where
 * `faroUrl` is empty, or pre-init in the bootstrap window).
 *
