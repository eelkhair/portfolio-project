...
export class ChatStore {
  ...
  readonly messages = signal<ChatMessage[]>([
    {
      ...
      html: marked.parse(this.welcomeMessage, { async: false }) as string,
      time: this.formatTime(),
    },
  ]);
  readonly loading = signal(false);
  readonly conversationId = signal<string | undefined>(undefined);
  readonly showJaeger = signal(
    this.isBrowser &&
  ...
  toggleJaeger(show: boolean): void {
    ...
    this.jaegerNotification.set(
      show ? 'debugging mode on' : 'debugging mode off',
    );
  ...
  send(text: string): Observable<void> {
    this.messages.update((msgs) => [
      ...msgs,
      { role: 'user', content: text, time: this.formatTime() },
    ]);
          ...
          error: () => {
            this.messages.update((msgs) => [
              ...msgs,
              {
                ...
                html: '<p>Unable to reach the AI service. Please try again later.</p>',
                time: this.formatTime(),
              },
            ]);
            this.loading.set(false);
