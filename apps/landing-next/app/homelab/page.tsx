  ...
  return (
    ...
    >
      ...
      <CsSection id="architecture">
        <h2>Architecture</h2>
        <p>The homelab runs on a Proxmox cluster with purpose-built hosts. Each has a static LAN IP and one job:</p>
        <ul className="cs-tradeoff-list">
          ...
          <li><strong>192.168.1.151</strong> &mdash; build infrastructure. Two GitHub Actions runners (labeled <code>proxmox-dev</code> and <code>proxmox-prod</code>), the private Docker registry at <code>jobboard-registry.elkhair.tech</code>, and a Docker Hub pull-through cache. Dedicated so builds never compete with running services on dev/prod.</li>
          <li><strong>192.168.1.150</strong> &mdash; general infrastructure. Nginx Proxy Manager (LAN TLS termination), Cloudflare tunnel daemon, WireGuard, Portainer, Uptime Kuma. This is the Proxmox host itself running Docker alongside the hypervisor.</li>
          <li><strong>192.168.1.115</strong> &mdash; HashiCorp Vault (<code>:8200</code>) + Grafana (<code>:3000</code>, exposed publicly at <code>grafana.elkhair.tech</code>). Vault is the single source of truth for secrets (DB credentials, Keycloak client secrets, API keys, OAuth2 client credentials), and is configured with transit-seal auto-unseal so reboots don&apos;t require manual intervention. Every Dapr sidecar mounts a <code>secretstores.hashicorp.vault</code> component pointed here.</li>
      ...
      <CsSection id="behind-scenes">
        ...
        <p><strong>Isolated build infrastructure.</strong> Two GitHub Actions runners live on a dedicated VM (<code>192.168.1.151</code>) — one labeled <code>proxmox-dev</code>, one labeled <code>proxmox-prod</code>. Builds no longer compete for CPU, RAM, or disk I/O with running services on the application hosts. From the runner VM, deploys reach the target hosts over <strong>Docker contexts via SSH</strong> (<code>docker --context job-board-dev compose up -d</code>), with SSH <code>ControlMaster</code> multiplexing so compose&apos;s parallel API calls share a single TCP connection instead of hammering <code>sshd</code>.</p>
        <p><strong>Environment-isolated image tags.</strong> Backend services push as <code>:dev</code> and <code>:prod</code> instead of a shared <code>:latest</code>. Before this, both environments raced to overwrite each other&apos;s <code>:latest</code> and a single corrupted manifest took both down. Now dev and prod have independent image lineages in the same registry.</p>
        <p><strong>Docker Hub pull-through cache.</strong> A second <code>registry:2</code> container on the build VM (port <code>5001</code>) is configured as a proxy to <code>registry-1.docker.io</code>. Dev and prod <code>/etc/docker/daemon.json</code> points <code>registry-mirrors</code> at it. Docker Hub rate limits or transient network blips no longer cascade into failed deploys — common images like <code>daprio/daprd</code> and <code>mcr.microsoft.com/dotnet/*</code> are served from LAN.</p>
        <p><strong>Private registry with garbage collection.</strong> The self-hosted registry at <code>jobboard-registry.elkhair.tech</code> runs with <code>REGISTRY_STORAGE_DELETE_ENABLED=true</code>, and a weekly cron runs <code>registry garbage-collect --delete-untagged</code> to reclaim untagged blob space. Without this, every CI build leaks blob storage forever &mdash; a common self-hosted-registry foot-gun.</p>
        ...
        <p><strong>Geo enrichment without rate limits.</strong> Every frontend (landing SSR, Angular admin, Angular public) attaches visitor country/city/region/lat/lon to telemetry spans for the &quot;Visitors by city&quot; map. Originally each call hit ipapi.co&apos;s free tier, which throttled at ~1000 requests/day per source IP &mdash; a container restart would wipe the in-memory cache and burn the day&apos;s budget in minutes. Now the gateway hosts <code>/api/public/geo</code> backed by a MaxMind GeoLite2 City database baked into the gateway image at build time. Lookups are sub-microsecond, cost nothing, never rate-limit, and a weekly GitHub Actions cron pulls the latest mmdb. All three frontends point at the gateway for geo &mdash; one mmdb, one source of truth, no external dependency.</p>
        <p><strong>Vault auto-unseal.</strong> A second small Vault instance on <code>192.168.1.150</code> acts as a transit-seal backend; the main Vault on <code>192.168.1.115</code> wraps its master key with the transit Vault&apos;s key. After a reboot, the main Vault calls the transit Vault to unwrap and unseal itself &mdash; no manual intervention, no <code>vault operator unseal</code> ceremony. Costs one extra container; eliminates the &quot;Vault is sealed and nothing can read its secrets&quot; cascade that takes down every service depending on it.</p>
      ...
      <CsSection id="decisions" alt>
        ...
        <CsDecisionGrid decisions={[
          ...
          { title: "Dedicated build VM over runners on the app hosts", content: "<p><strong>Why:</strong> Originally the GitHub Actions runners lived on the dev and prod application VMs. Every <code>docker build</code> for the .NET monorepo competed with running services for CPU, RAM, and disk I/O — request latency would spike during a deploy. Moving runners to a single dedicated VM (along with the registry and Docker Hub pull-through cache) isolates build load from runtime load entirely. Deploys reach the app hosts via Docker contexts over SSH, with <code>ControlMaster</code> multiplexing.</p><p><strong>Alternative:</strong> Keep runners on the app hosts and accept the build/runtime contention. Or use GitHub-hosted runners — but that loses the local layer cache, the local registry pull-through, and the BuildKit cache reuse that make subsequent builds quick.</p>" },
          { title: "Centralized geo enrichment on the gateway", content: "<p><strong>Why:</strong> Every frontend needs visitor country/city/region for span tagging. Doing it per-app meant landing called ipapi.co (rate-limited), and Angular apps called landing's <code>/api/geo</code> (same dependency, doubled). Putting MaxMind on the gateway gives one mmdb, one HTTP endpoint, zero external dependencies. Landing SSR also calls the gateway over the LAN bridge network — adds 1-2 ms but eliminates the rate-limit failure mode.</p><p><strong>Alternative:</strong> mmdb in every container. Wastes ~65 MB per image, and Next.js's edge runtime transformer strips runtime imports needed to load it from inside the landing app anyway.</p>" },
        ]} />
      </CsSection>
