...
namespace JobBoard.AI.Infrastructure.Dapr.AITools.Admins.Micro.Jobs;

public static class CreateJobTool
{
    public static AIFunction Get(IActivityFactory activityFactory, IAdminApiClient client, ILoggerFactory loggerFactory, IAiNotificationHub notificationHub, IUserAccessor userAccessor)
    {
        ...
        return AIFunctionFactory.Create(
            async Task<object> (Guid companyId, string draftId, bool deleteDraft, CancellationToken ct) =>
            {
                ...
                var draftResponse = await client.GetDraftByIdAsync(id, ct);
