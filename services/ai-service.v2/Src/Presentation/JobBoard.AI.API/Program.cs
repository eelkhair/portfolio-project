...
(await builder.AddDaprServices("ai-service-v2")).ConfigureLogging("ai-service-v2").AddCustomHealthChecks().Services
    ...
    .AddDiagnosticsServices(builder.Configuration, "ai-service-v2")
    .AddSignalR();

builder.Services
    .AddMonolithHttpClient(builder.Configuration)
    .AddScoped<IAiNotificationHub, AiNotificationHubNotifier>()
    .AddSingleton(new BlobServiceClient(
        builder.Configuration.GetConnectionString("BlobStorage") ?? "UseDevelopmentStorage=true"))
    .AddMcpToolProviders(builder.Configuration);
