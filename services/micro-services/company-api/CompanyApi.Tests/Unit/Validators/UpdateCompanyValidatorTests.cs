...
/// <summary>
/// Tests the field-level validation rules from UpdateCompanyValidator.
/// The actual validator uses Resolve&lt;ICompanyDbContext&gt;() and Resolve&lt;IHttpContextAccessor&gt;()
/// for async DB checks. We replicate the field rules in a test-only AbstractValidator
/// to test them independently without FastEndpoints DI.
/// </summary>
[Trait("Category", "Unit")]
public class UpdateCompanyValidatorTests
{
    private sealed class FieldRulesValidator : AbstractValidator<UpdateCompanyRequest>
    {
    ...
    private static UpdateCompanyRequest ValidRequest() => new()
    {
        ...
        CompanyEmail = "update@company.com",
