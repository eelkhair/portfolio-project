...
public class DraftMapperTests
{
    ...
    [Fact]
    public void ToPayload_WithEmptyCollections_SerializesEmptyArrays()
    {
        var evt = new DraftSavedV1Event(
            UId: Guid.NewGuid(),
            CompanyUId: Guid.NewGuid(),
    ...
    [Fact]
    public void ToPayload_IdInContentJson_MatchesDraftId()
    {
        var draftId = Guid.NewGuid();
        var evt = new DraftSavedV1Event(
            UId: draftId,
        ...
        var json = JsonDocument.Parse(result.ContentJson);
        json.RootElement.GetProperty("id").GetString().ShouldBe(draftId.ToString());
        json.RootElement.GetProperty("notes").GetString().ShouldBe("Test notes");
    }

    [Fact]
    public void ToPayload_WithMultipleResponsibilities_SerializesAll()
    {
        var evt = new DraftSavedV1Event(
            UId: Guid.NewGuid(),
            CompanyUId: Guid.NewGuid(),
            ...
            Qualifications: ["C#", "SQL", "Docker"]
        );

        var result = DraftMapper.ToPayload(evt);

        var json = JsonDocument.Parse(result.ContentJson);
        json.RootElement.GetProperty("responsibilities").GetArrayLength().ShouldBe(4);
        json.RootElement.GetProperty("qualifications").GetArrayLength().ShouldBe(3);
    }
}
